...
function editar(id) {
    $.ajax({
        ...
        success: function (w) {
            ...
            $("#editar_descripcion").val(w.pais);
            $("#idpost").val(w.ideditorial);
        }
    });
        ...
        callback: function (result) {
            if (result) {
                $.ajax({
                    ...
                    success: function (w) {
                        bootbox.alert({
                            message: "Editorial eliminado de la lista!!!",
                            callback: function () {
                ...
                bootbox.alert({
                    message: "Editorial no eliminado!",
                    size: 'small'
                });
            }
        }
...
$("#guardar").click(function () {
    ...
    $.ajax({
        ...
        data: JSON.stringify({'nombre': titulo, 'pais': desc}),
        cache: false,
        success: function (w) {
            bootbox.alert({
                message: "Registro de Editorial Guardado en la lista!!",
                callback: function () {
                    console.log('This was logged in the callback!');
                }
            });
        ...
        callback: function (result) {
            if (result) {
                $.ajax({
                    ...
                    type: 'PUT',
                    contentType: "application/json; charset=utf-8",
                    data: JSON.stringify({'ideditorial': id, 'nombre': titulo, 'pais': desc}),
                    cache: false,
                    success: function (w) {
                        bootbox.alert({
